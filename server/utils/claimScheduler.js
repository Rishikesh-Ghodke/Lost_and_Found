  ...
  try {
    ...
    for (const item of expiredItems) {
      ...
      if (item.status === 'claimed' && item.claims && item.claims.length > 0) {
        ...
        // Set new verification time for 24 hours from now (only for items that had claims)
      ...
      // If item is already available with no claims, don't update verification time
      // This prevents old items from having their verification time constantly updated
    }
